//
//  ImageGeneratorFromString.swift
//  ChannelizeUI
//
...
struct ImageFromStringProvider {
    var cacheKey: String { return UUID().uuidString }
    let letter: String
    let imageSize: CGSize
    
    init(name: String, imageSize: CGSize) {
        self.letter = name
        self.imageSize = imageSize
    }
    
    private func initialsFromString(string: String) -> String {
        if(string.components(separatedBy: .whitespacesAndNewlines).count > 1){
            return string.components(
                separatedBy: .whitespacesAndNewlines).reduce("") {
                    return ($0.isEmpty ? "" : "\($0.uppercased().first!)") +
                        ($1.isEmpty ? "" : "\($1.uppercased().first!)")
            }
        }else{
    ...
    private func pickColor(alphabet: Character?) -> UIColor {
        guard let newAlphabet = alphabet else {
            return UIColor.customSystemTeal
        }
        ...
        let str = String(newAlphabet).unicodeScalars
    ...
    func generateImage(with fontSize: CGFloat = 17.0) -> UIImage? {
        ...
        UIGraphicsBeginImageContextWithOptions(lblNameInitialize.frame.size, true, scale)
...
extension UIImageView {
    ...
    open func setImage(string: String?,
                       ...
                       circular: Bool = false,
                       stroke: Bool = false,
                       textAttributes: [NSAttributedString.Key: Any]? = nil, size: CGSize = .zero) {
    ...
    private func imageSnap(text: String?,
        ...
        var size = size == .zero ? bounds.size : size
